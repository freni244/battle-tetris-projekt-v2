  ...
  (cond
    ((empty? (cdr items))
     (cond ((= (car items) 0)
           ...
           ((= (car items) 1)
            (draw-grid canvas dc x y 20 20 "lime"))
           ((= (car items) 2)
            (draw-grid canvas dc x y 20 20 "blue"))
           ((= (car items) 3)
            (draw-grid canvas dc x y 20 20 "red"))
           ((= (car items) 4)
            (draw-grid canvas dc x y 20 20 "yellow"))
           ((= (car items) 5)
            (draw-grid canvas dc x y 20 20 "orange"))
           ((= (car items) 6)
            (draw-grid canvas dc x y 20 20 "cyan"))
           ((= (car items) 7)
            (draw-grid canvas dc x y 20 20 "magenta"))))    ;; tillfällig lösning
    ((= (car items) 0)
